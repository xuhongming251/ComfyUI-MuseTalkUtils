...
vae_module = vae.VAE(model_path = os.path.join(model_path, "sd-vae-ft-mse"))

class MuseTalkTrainPreprocess:
    def __init__(self):
        pass
    ...
    def preprocess(self, origin_images, pose_kps, crop_type, top_reserve, bottom_reserve, left_reserve, right_reserve):
        ...
        musetalk_global_data.faces_latent_list = []
        musetalk_global_data.resized_cv2_frame_list = []
        ...
        for image, landmark in zip(origin_images, musetalk_global_data.origin_face_landmarks):
            ...
            if len(landmark) == 0:
                if len(musetalk_global_data.rotated_faces) > 0:
                    ...
                    # TODO: process no face first frame
                    continue
            ...
            musetalk_global_data.resized_cv2_frame_list.append(resized_rotated_face)
            ...
            cv2.line(rotated_face_with_landmark, (width//2, 0), (width//2, height-1), (255, 0, 0), 1)  # v line, blue
            cv2.line(rotated_face_with_landmark, (0, height//2), (width-1, height//2), (255, 0, 0), 1)  # h line, blue
        ...
        frame_count = len(musetalk_global_data.resized_cv2_frame_list)
        pbar  = comfy.utils.ProgressBar(frame_count)

        print("frame_count", frame_count)

        for fid in range(frame_count):
            gt_face = musetalk_global_data.resized_cv2_frame_list[fid]
            rand_num = random.randint(0, frame_count-1)
            ref_face = musetalk_global_data.resized_cv2_frame_list[rand_num]
            latents = vae_module.get_train_latents_for_unet(gt_face, ref_face)
            musetalk_global_data.faces_latent_list.append(latents)
